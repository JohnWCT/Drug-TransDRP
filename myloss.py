...
def Adversarial_loss(domain_s, domain_t, BCEloss):
    ...
    target_labels = torch.from_numpy(np.array([[0]]*bs_s + [[1]]*bs_t).astype('float32')).to(domain_s.device)
...
class InfoMax_loss(nn.Module):
    ...
    def process(self, data_type, p_key, p_value):
        ...
        for i in data_type:
            ...
            if (len(idx[0]) != 0):
                ...
                summary.append(torch.zeros(1, self.hidden_dim).to(p_value.device))
    ...
    def forward(self, s_feat, t_feat, s_type, t_type, prototype):
        ...
        pos_summary_t = self.process(t_type, batch_s[0].to(t_feat.device), batch_s[1].to(t_feat.device))
        neg_summary_t = self.process(t_type, batch_t[0].to(t_feat.device), batch_t[1].to(t_feat.device))
