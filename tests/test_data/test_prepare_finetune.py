...
def test_prepare_finetune_data(tmp_data_dir):
    config = TransDRPMultilabelConfig(
        ...
        source_response_path=os.path.join(tmp_data_dir, "source_response.csv"),
        target_response_path=os.path.join(tmp_data_dir, "target_response.csv"),
        ...
        method="transdrp_ft",
        pretrain_checkpoint="dummy_ckpt.pt",
        output_dir="dummy",
        overwrite=True,
        batch_size=4,
        epochs=1,
        ...
        source_cancer_type_path=os.path.join(tmp_data_dir, "source_cancer_types.csv"),
        target_cancer_type_path=os.path.join(tmp_data_dir, "target_cancer_types.csv"),
        cancer_type_col="Cancer_type",
        drug_smiles_path=os.path.join(tmp_data_dir, "drug_smiles.csv"),
        alph=0.2,
        beta=0.3,
        latent_dim=8,
    ...
    prepared = prepare_finetune_data(config)
    assert prepared.drug_index.drug_ids == ["druga", "drugb", "drugc"]
    assert len(prepared.folds) == 2
    assert prepared.cancer_type_table is not None
